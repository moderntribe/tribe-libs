...
class Walker_Flat_Category_Checklist extends Walker_Category_Checklist {
	...
	public function start_el( &$output, $category, $depth = 0, $args = [], $id = 0 ) {
		...
		$output .= "\n<li id='{$taxonomy}-{$category->term_id}'$class>" .
			'<label class="selectit"><input value="' . ( $hierarchical ? $category->term_id : $category->slug ) . '" type="checkbox" name="'.$name.'[]" id="in-'.$taxonomy.'-' . $category->term_id . '"' .
